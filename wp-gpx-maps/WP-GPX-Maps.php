	    ...
	    $.fn.wpgpxmaps = function( params ) {
			...
			*/
			...
			<?php
				$conn = db_connect();
				$query = "
					SELECT * FROM puntiSignificativi
				 	WHERE tipo != 'altro';
				";
				$res = mysql_query($query);
				while ($row = mysql_fetch_array($res)) {
					echo "locations.push(
						['<h5>".$row["nome"]."</h5><p class=w3-large>'+
						  'Punto di ".$row["tipo"]."<br/>".
						  $row["sitoWeb"]."</p>',
						".$row["latitudine"].",
						".$row["longitudine"]."]);";
				}
			 ?>
			// controlla se la finestra è aperta
			 function isOpen(infoWindow){
				var map = infoWindow.getMap();
				return (map !== null && typeof map !== "undefined");
			 }
			//  aggungo i marker dei punti significativi
			 for (i = 0; i < locations.length; i++) {
				...
				//  quando clicco sul marker apro/chiudo la finestra
			     google.maps.event.addListener(mymarker, 'click', (function (marker, i, infowindow) {
			         return function () {
